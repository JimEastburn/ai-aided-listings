...
function MyApp({ Component, pageProps }) {
  ...
  const { data:autolistData, error:autolistError, isLoading:autolistIsLoading } = useSWR(() => (gPT3data ? 
    `https://www.autolist.com/api/v2/search?page=1
    &make=${gPT3data.answer.make}
    &model=${gPT3data.answer.model}
    &year_max=${gPT3data.answer.year}
    &year_min=${gPT3data.answer.year}` : null), fetcher);
